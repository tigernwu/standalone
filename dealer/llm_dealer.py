...
class TradePositionManager:
    ...
    def check_stop_loss(self, current_price: float) -> bool:
        ...
        if current_position > 0 and current_price <= self.stop_loss:
            return True
        elif current_position < 0 and current_price >= self.stop_loss:
            return True
...
class LLMDealer:
    def __init__(self, llm_client, symbol: str,data_provider: MainContractProvider,trade_rules:str="" ,
        ...
        self.current_trade_plan = None  # 存储当前的交易计划
    ...
    def _prepare_llm_input(self, bar: pd.Series, news: str) -> str:
        ...
        if current_position > 0:
            position_description = f"多头 {current_position} 手"
        elif current_position < 0:
            position_description = f"空头 {abs(current_position)} 手"
        
        if self.current_trade_plan:
            position_description += f"\n当前交易计划: {self.current_trade_plan}"
        ...
        # 添加趋势识别
        trend = self._identify_trend(self.today_minute_bars)
        trend_info = f"当前市场趋势: {trend}"

        # 添加动态止损信息
        stop_loss_info = self._get_dynamic_stop_loss_info()

        # 添加多时间周期分析
        multi_timeframe_analysis = self._get_multi_timeframe_analysis()

        # 添加市场情绪指标
        market_sentiment = self._get_market_sentiment()

        # 添加交易历史摘要
        trade_history_summary = self._summarize_trade_history()
        ...
        请仔细分析上述交易历史，考虑以下几点：
        1. 我们的交易策略是否在当前市场环境中有效？
        2. 是否存在连续亏损的情况？如果是，可能的原因是什么？
        3. 我们是否错过了一些重要的市场信号或转折点？
        4. 基于历史交易，当前的市场状况是否适合继续交易，还是应该暂时观望？

        请根据上述信息，特别是最近的交易历史，分析当前市场状况和自身表现。如果发现连续亏损或者策略效果不佳，请考虑调整交易策略。

        请注意：
        1. 日内仓位需要在每天15:00之前平仓。夜盘根据合约不同，交易停止时间不同。如果你能确定合约的夜盘停止时间，在交易停止之前平仓。
        2. 当前时间为 {bar['datetime'].strftime('%H:%M')}，请根据时间决定是否需要平仓。
        3. 交易指令必须严格遵循以下格式之一：
        - 买入开仓：'buy 数量'（例如：'buy 2' 或 'buy all'）
        - 卖出开仓：'short 数量'（例如：'short 2' 或 'short all'）
        - 卖出平多：'sell 数量'（例如：'sell 2' 或 'sell all'）
        - 买入平空：'cover 数量'（例如：'cover 2' 或 'cover all'）
        - 持仓不变：'hold'
        4. 当前持仓已经达到最大值或最小值时，请勿继续开仓。
        5. 交易计划必须与交易指令保持一致，并包含以下内容，使用以下格式：
        - 交易指令：[与上述格式完全一致的指令]
        - 止损价格：[数字]
        - 目标价格：[数字]
        - 持仓时间：[数字]小时或[数字]分钟
        - 风险评估：[低/中/高]
        - 预期盈利：[数字]点
        6. 如果选择持仓不变（hold），也必须提供完整的交易计划，包括继续持有的理由和预期。
        7. 如果要修改现有的交易计划，请使用"更新交易计划："后跟新的完整计划详情。

        请根据以上信息，给出交易指令并提供完整的交易计划。
        请以JSON格式输出，包含以下字段：
        - trade_instruction: 交易指令（字符串，必须严格遵循上述格式）
        - trade_reason: 此刻交易决策的详细理由（字符串）
        - trade_plan: 完整的交易计划，必须包含上述第5点中提到的所有内容（字符串）
        - next_message: 下一次需要关注的重要信息（字符串）

        请确保输出的JSON格式正确，并用```json 和 ``` 包裹。交易指令必须与交易计划中的指令完全一致。
        """
        return input_template

    def _get_multi_timeframe_analysis(self) -> str:
        if self.minute_history.empty:
            return "Insufficient data for multi-timeframe analysis"

        try:
            # Ensure datetime column is present and set as index
            if 'datetime' not in self.minute_history.columns:
                return "Datetime column missing in minute history data"

            df = self.minute_history.set_index('datetime')

            # Calculate RSI for different timeframes
            rsi_5m = self._calculate_rsi(df.resample('5T').last(), 14)
            rsi_15m = self._calculate_rsi(df.resample('15T').last(), 14)
            
            return f"5分钟RSI: {rsi_5m:.2f}, 15分钟RSI: {rsi_15m:.2f}"
        except Exception as e:
            self.logger.error(f"Error in multi-timeframe analysis: {str(e)}")
            return "Error in multi-timeframe analysis"
    ...
    def _parse_llm_output(self, llm_response: str) -> Tuple[str, Union[int, str], str, str, str]:
        ...
        try:
            ...
            if trade_plan.startswith("更新交易计划："):
                self.current_trade_plan = trade_plan[7:]  # 更新当前的交易计划
            elif action in ['buy', 'short']:
                self.current_trade_plan = trade_plan  # 开仓时设置新的交易计划
            elif action in ['sell', 'cover']:
                self.current_trade_plan = None  # 平仓时清除交易计划
            # 解析交易指令和数量
